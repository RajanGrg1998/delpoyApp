...
class HomeDemoApp extends StatefulWidget {
  const HomeDemoApp({Key? key}) : super(key: key);

  @override
  State<HomeDemoApp> createState() => _HomeDemoAppState();
}

class _HomeDemoAppState extends State<HomeDemoApp> with WidgetsBindingObserver {
    ...
    if (isLoading) {
      ...
      return CupertinoPageScaffold(
        ...
        child: oreintation == Orientation.landscape
            ? buildLandscape(clipCon, oreintation)
            : buildPotrait(clipCon, oreintation),
      );
    }
  }

  CameraPreview cameraPreview({Widget? widget}) {
...
//potrait
  buildPotrait(ClipController clipCon, Orientation oreintation) {
    return Column(
      children: [
        Expanded(
          child: cameraPreview(
                          ...
                          ? SizedBox(
                              ...
                              child: RawMaterialButton(
                                onPressed: () async {
                                  ...
                                  // }
                                  if (clipCon.isLastSeconButtonPressed ==
                                      false) {
                                    return _showMyDialog(
                                        context, _videoFile!.path);
                                  } else {
                        ...
                        CustomTimeButton(
                          ...
                          onPressed: () async {
                            if (isRecordingInProgress) {
                              // XFile? rawVideo = await stopVideoRecording();
                              clipCon.changeLastSecondButtonPressed(true);
                              XFile? rawVideo = await stopVideoRecording();
                              ...
                              double duration = controller
                                  ...
                                  .toDouble();
                              double last5Sec = (duration - 5.0);
                              LastClipController().saveLastClipVideo(
                                  startValue: last5Sec,
                                  endValue: duration,
                                  onSave: (outcome) async {
                                    clipCon.addItem(
                                        VideoFileModel(videoPath: outcome));
...
//lanscape
  buildLandscape(ClipController clipCon, Orientation oreintation) {
    return Row(
      children: [
        Expanded(
          child: Container(
            ...
            CupertinoDialogAction(
                ...
                onPressed: () {
                  ...
                  Provider.of<ClipController>(context, listen: false)
                      .changeLastSecondButtonPressed(false);
                }),
            CupertinoDialogAction(
